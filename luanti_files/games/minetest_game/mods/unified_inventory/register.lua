	...
	action = function(player)
		...
		if minetest.check_player_privs(player_name, {home=true}) then
			...
			if is_sethome_protect(position) then
				minetest.chat_send_player(player_name, "** It is forbidden to set up a home position in this area! **")
				return
			end	
...
ui.register_button("home_gui_go", {
	...
	action = function(player)
		...
		if minetest.check_player_privs(player_name, {home=true}) then
			...
			if is_sethome_protect(position) then
				minetest.chat_send_player(player_name, "** You cannot go home from this territory! **")
				return
			end	
...
-- Leaderboard button (replacing set day position)
ui.register_button("leaderboard", {
	type = "image",
	image = "default_book_written.png",
	tooltip = "Server Leaderboard",
	action = function(player)
		local player_name = player:get_player_name()
		local cmd = minetest.registered_chatcommands["leaderboard"]
		if cmd then
			cmd.func(player_name, "")
		end
	end,
})

-- Factions button (swapped with set day position)
ui.register_button("factions", {
	type = "image", 
	image = "default_book.png",
	tooltip = "Factions Menu",
	action = function(player)
		local player_name = player:get_player_name()
		if factions and factions.show_menu then
